...
# Install required programs on debian or derived distributions
	...
	sudo -S apt-get -y install binutils bzip2 curl gcc pkg-config libpng-dev libjpeg-dev diffutils git make subversion patch python tar texinfo wget
elif program_exists yum && program_exists sudo; then
	sudo -S yum -y install binutils bzip2 curl gcc pkgconfig libpng-devel libjpeg-devel diffutils git make subversion patch python tar texinfo wget
# Install required programs on gentoo or derived distributions
elif program_exists emerge && program_exists sudo; then
	sudo -S emerge -an sys-devel/binutils app-arch/bzip2 net-misc/curl sys-devel/gcc dev-util/pkgconfig media-libs/libpng media-libs/libjpeg-turbo sys-apps/diffutils dev-vcs/git sys-devel/make dev-vcs/subversion sys-devel/patch dev-lang/python app-arch/tar sys-apps/texinfo net-misc/wget
# Catch-all, installs requirements when no package manager is available
else
	echo "Checking for zlib, libjpeg and libpng.."
	[ -d /usr/local/src ] || sudo -S mkdir -p /usr/local/src

	if ([ ! -f /usr/include/zlib.h ] || [ ! -f /usr/lib/libz.so ]) && ([ ! -f /usr/local/include/zlib.h ] || [ ! -f /usr/local/lib/libz.so ]); then
		[ -d http://zlib.net/zlib-1.2.8.tar.gz ] || curl 'http://zlib.net/zlib-1.2.8.tar.gz' | sudo -S tar xz -C /usr/local/src
		echo "Building zlib"
		cd /usr/local/src/zlib-1.2.8
		sudo -S ./configure
		sudo -S make -j $CORE_COUNT install
	else
	...
	if ([ ! -f /usr/include/jpeglib.h ] || [ ! -f /usr/lib/libjpeg.so ]) && ([ ! -f /usr/local/include/jpeglib.h ] || [ ! -f /usr/local/lib/libjpeg.so ]); then
		[ -d /usr/local/src/jpeg9b ] || curl 'http://www.ijg.org/files/jpegsrc.v9b.tar.gz' | sudo -S tar xz -C /usr/local/src
	...
	if ([ ! -f /usr/include/png.h ] || [ ! -f /usr/lib/libpng.so ]) && ([ ! -f /usr/local/include/png.h ] || [ ! -f /usr/local/lib/libpng.so ]); then
		[ -d /usr/local/src/libpng-1.6.25 ] || curl -L 'http://download.sourceforge.net/libpng/libpng-1.6.25.tar.gz' | sudo -S tar xz -C /usr/local/src
...
if program_exists sudo; then
	sudo -S mkdir -p    "$INSTALL_DIR"
	sudo -S chmod o+rwx "$INSTALL_DIR"
else
	mkdir -p "$INSTALL_DIR"
fi
...
if ! program_exists "$INSTALL_DIR/sh-elf/bin/sh-elf-gcc" || ! program_exists "$INSTALL_DIR/sh-elf/bin/sh-elf-g++"; then
	...
	echo "Downloading.."; bash download.sh
	echo "Unpacking..";   bash unpack.sh
